...
describe Puppet::Type.type(:grafana_notification) do
  ...
  context 'when grafana_password is marked sensitive' do
    it 'marks the grafana_password parameter sensitive and does not warn' do
      ...
      expect(notification.parameter(:grafana_password).sensitive).to be true
      expect(notification[:grafana_password]).to eq('admin')
    ...
    it 'does not mark the grafana_password parameter sensitive when not requested' do
      notification = described_class.new(name: 'foo', grafana_url: 'http://example.com', grafana_password: 'admin')

      expect(notification.parameter(:grafana_password).sensitive).to be_falsey
    end
  end
end
